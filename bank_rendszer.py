class Bank:
    ...
    def add_new_bankoffice(self,bank_nev, fiok_nev):
    ...
    def check_bankoffice_name(self, bankiroda_nev):
        for office in self.bankirodak:
            if office[0] == bankiroda_nev:
                return True
        return False
...
def main():
    ...
    while choice != str(8):
        ...
        if choice == "1":
            ...
            bank_nev = input("Adja meg a bank nevét: ")
            if bank.check_bank_name(bank_nev):
                bankfiok_nev = input("Adja meg a bankfiók nevét: ")
                bank.add_bankoffices(bank_nev, bankfiok_nev)
                print("Sikresen hozzá adta a bankfiókot.")
            else:
                print("Az ön által megadott bank nem taláható Hozzá akja adni?")
                answer = input("Írjon 'igen'-t vagy 'nem'-et: ")
                if answer == "igen":
                    location = input("Adja meg a bank helyét: ")
                    bank.add_new_bank(bank_nev, location)
            ...
            bankfiok_nev = input("Adja meg a bankfiók nevét: ")
            if bank.check_bankoffice_name(bankfiok_nev):
                bank.del_bankoffices(bankfiok_nev)
                print("Sikresen kitörölte a bankfiókot.")
            else:
                print("Az ön által megadott bankfiók nem található.")
            ...
            if bank.check_bank_name(bank_nev):
                ...
                if bank.check_bankoffice_name(bankfiok_nev):
                    ...
                    print("Az ön által megadott bankfiók nem található. Létre akarja hozni?")
                    answer = input("Írjon 'igen'-t vagy 'nem'-et: ")
                    if answer == "igen":
                        bank.add_new_bankoffice(bank_nev,bankfiok_nev)
                        customer.add_customer(nev, address, bank_nev, bankfiok_nev)
                        print("Sikeresen felvette az új ügyfelet a rendszerhez.")
                    else:
                            print("Az új ügyfél felvétele sikertelen volt.")
            else:
                print("A bank nem található. Hozzá akarja adni?")
                answer = input("Írjon 'igen'-t vagy 'nem'-et: ")
                if answer == "igen":
                    location = input("Adja meg a bank helyét: ")
                    bank.add_new_bank(bank_nev, location)
